...
object Option:
  ...
  def map2_alt[A, B, C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] =
    a.flatMap(fa => b.map(fb => f(fa, fb)))
  ...
  def traverse_folding[A, B](as: List[A])(f: A => Option[B]): Option[List[B]] =
    as.foldRight[Option[List[B]]](Some(Nil)) { (a, acc) =>
      map2(f(a), acc)(_ :: _)
    }

  def sequence_from_traverse[A](as: List[Option[A]]): Option[List[A]] =
    traverse(as)(a => a)
